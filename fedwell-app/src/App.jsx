...
export default function App() {
  ...
  if(!role) return(
    <div className="fw-page fw-login-wrap">
      ...
      {/* Right login panel */}
      <div style={{background:isDark?"#1E293B":"#F8FAFC",display:"flex",alignItems:"center",justifyContent:"center",padding:"36px 24px"}}>
        <div style={{width:"100%",maxWidth:390}}>
          {/* Mini Fed Uni logo on mobile (left panel hidden) */}
          <div style={{marginBottom:20,display:"flex",justifyContent:"center"}}>
            <FedUniLogo white={isDark} height={34}/>
          </div>
          <div style={{fontSize:22,fontWeight:800,color:"var(--text)",marginBottom:4}}>Sign in to FedWELL</div>
          ...
          <button onClick={doLogin}
            style={{width:"100%",padding:13,borderRadius:10,background:`linear-gradient(135deg,${FED_NAVY},#003476)`,color:"white",border:"none",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 14px rgba(0,32,96,0.35)`}}>
            Sign In →
          </button>
          <div style={{marginTop:14,padding:"11px 13px",background:isDark?"#243048":"#F1F5F9",borderRadius:9,fontSize:12,color:"var(--muted)",lineHeight:1.8}}>
          ...
          <SCard gradient={`linear-gradient(135deg,${FED_NAVY},#003476)`} icon="👤" label="Patient Details">
            ...
            <div style={{marginBottom:14}}>
              ...
              <div className="fw-gender-row">
                {PRESET.map(g=>{const sel=f.gender===g&&!isOther;return(
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8}}>
                {["Yes","No"].map(v=>{const sel=f.seenGP===v,c=v==="Yes"?"#15803D":"#B45309";return(
                        ...
                        {AGE_GROUPS.map(g=>(
                          <div key={g.l} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                            ...
                            <div style={{flex:1,background:"var(--border)",borderRadius:999,height:8,overflow:"hidden"}}>
                              <div style={{width:`${pct(pc.ageDist[g.l]||0,pc.count)}%`,height:8,
