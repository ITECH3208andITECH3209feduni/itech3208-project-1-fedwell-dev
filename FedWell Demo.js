            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {["Male", "Female", "Non-Binary", "Prefer not to say", "Other"].map(g => (
                <button key={g} onClick={() => setClient(c => ({ ...c, gender: g }))}
                  style={{
                    padding: "8px 16px", borderRadius: 20, border: `2px solid ${client.gender === g ? THEME.blue : "#e2e8f0"}`,
                    background: client.gender === g ? THEME.light : "white", color: client.gender === g ? THEME.navy : THEME.grey,
                    cursor: "pointer", fontSize: 13, fontWeight: client.gender === g ? 600 : 400
                  }}>{g}</button>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["Yes", "No"].map(v => (
                <button key={v} onClick={() => setClient(c => ({ ...c, seen_gp: v }))}
                  style={{
                    padding: "8px 24px", borderRadius: 20, border: `2px solid ${client.seen_gp === v ? (v === "Yes" ? THEME.teal : THEME.red) : "#e2e8f0"}`,
                    background: client.seen_gp === v ? (v === "Yes" ? THEME.mint : THEME.lightRed) : "white",
                    cursor: "pointer", fontSize: 14, fontWeight: 600,
                    color: client.seen_gp === v ? (v === "Yes" ? THEME.teal : THEME.red) : THEME.grey
                  }}>{v}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ ...styles.card, borderTop: `4px solid ${THEME.navy}`, background: "linear-gradient(150deg, rgba(255,255,255,0.95) 0%, rgba(232,240,248,0.6) 100%)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[
              ...
              <div key={label} style={{
                background: flagged ? THEME.lightRed : "#f8fafc", borderRadius: 8, padding: "10px 14px",
